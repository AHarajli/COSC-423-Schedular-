...
class Submittor extends Thread {
	 ...
	 *            = an array of Strings, each a line from the input file. The syntax
	 *            of this string will vary depending on the type of scheduler we are
	 *            implementing. For every type of scheduler the first token will be
	 *            the name of a job/process and the second token will be an integer
	 *            representing the number of msecs before this job should be
	 *            submitted to the OS. For a FCFS scheduler the remainder of the
	 *            string will consist of a single token, an integer equal to the
	 *            length of the simulated CPU burst in msec.
	 * @param s
	 *            = The kernel simulator
	 * @param progenitor
	 *            = a factory for creating JobWorkable objects that will be embedded
	 *            in the Job.
	 */
	public Submittor(ArrayList<String> jobDescriptions, SystemSimulator s, WorkFactory progenitor) {
		// Each element of jobDescriptions is a String consisting of two words: a job
		// number/name and
	...
	/*
	 * Sleeps for a bit, creates work to be accomplished in a Job and submits this
	 * and a description of the work to its system simulator, and repeats. See my
	 * boilerplate code, Submittor.java.
	 */
	public void run() {
		// iterate through jobs strings, parse the string, create jobs and add to
		// operating system.
